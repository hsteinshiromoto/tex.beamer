...
% Body typeface. TeX Gyre Heros is URW Nimbus Sans, metrically identical to Helvetica,
% so a machine without Helvetica installed still gets the same measure and rhythm.
...
% Chrome typeface: headline, footline and footnotes. Jost* is a Futura revival, vendored
% under the OFL in src/fonts/ so the deck builds identically everywhere, CI included.
...
\setbeamerfont{footnote}{family=\headerfont,size=\scriptsize}
\setbeamerfont{footnote mark}{family=\headerfont}
...
% Frametitle
% ---
% Flat band, no shadow, and the title centered in it. The stock template pulls the title up
% with \vbox{}\vskip-1ex and cancels the bottom padding with \vskip-.3cm. That second skip
% only runs when no optional argument is given, and this template needs one, so the title sat
% low. Symmetric \sep and no vertical skips center it instead. The struts keep the band the
% same height whether or not the title has a descender.
\makeatletter
\setbeamertemplate{frametitle}{%
  ...
  % sep pads all four sides, so it alone cannot set the vertical and the horizontal
  % padding apart. sep gives the vertical padding, and leftskip tops the left side back
  % up to the 0.3cm the stock template uses.
  \begin{beamercolorbox}[sep=0.15cm,leftskip=0.15cm,wd=\the\@tempdima,%
                         colsep=-4bp,rounded=false,shadow=false]{frametitle}%
...
% Headline
% ---
% Flat bar: \insertnavigation draws the section names together with the mini frames
% that show how far into the deck the current slide sits.
\setbeamercolor{section in head/foot}{fg=white,bg=ggrey900}
\setbeamercolor{mini frame}{fg=ggrey500,bg=ggrey900}
\setbeamerfont{section in head/foot}{family=\headerfont,size=\tiny}
\setbeamersize{mini frame size=1.6mm}

% \allcaps is the letterspaced uppercase helper defined in conf/commands.tex
\setbeamertemplate{section in head/foot}{\allcaps{\insertsectionhead}}
\setbeamertemplate{section in head/foot shaded}{%
  \color{ggrey500}\allcaps{\insertsectionhead}}

\setbeamertemplate{headline}{%
  \begin{beamercolorbox}[wd=\paperwidth,ht=2.3ex,dp=2.7ex]{section in head/foot}%
    \hspace*{1.5em}\insertnavigation{\dimexpr\paperwidth-3em\relax}%
  \end{beamercolorbox}%
}
...
% Footline
% ---
\setbeamercolor{footline}{fg=ggrey400,bg=ggrey900}
\setbeamerfont{footline}{family=\headerfont,size=\tiny}

% Deck position, drawn as the top edge of the footline band. Guarded because
% \inserttotalframenumber defaults to 1 until the .aux is read, so the frame number can
% exceed it on the first pass.
\newcommand{\beamerprogressbar}{%
  \ifnum\insertframenumber<\inserttotalframenumber
    \textcolor{gblue500}{%
      \rule{\dimexpr\paperwidth*\insertframenumber/\inserttotalframenumber\relax}{1.6pt}}%
    \textcolor{ggrey700}{%
      \rule{\dimexpr\paperwidth-\paperwidth*\insertframenumber/\inserttotalframenumber\relax}{1.6pt}}%
  \else
    \textcolor{gblue500}{\rule{\paperwidth}{1.6pt}}%
  \fi
}
...
\defbeamertemplate*{footline}{my footline}
{
   \ifnum \insertpagenumber=1
    ...
    \else
  \leavevmode%
  % One vbox so \nointerlineskip is legal here: it assigns \prevdepth, which needs
  % vertical mode, and it is what keeps the bar flush against the band below it.
  \vbox{%
  \hbox{\beamerprogressbar}%
  \nointerlineskip
  \hbox{%
  \begin{beamercolorbox}[wd=.34\paperwidth,ht=2.4ex,dp=1.6ex,left]{footline}%
    \hspace*{1.5em}\usebeamerfont{footline}\insertshortauthor
  \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=.33\paperwidth,ht=2.4ex,dp=1.6ex,center]{footline}%
    \usebeamerfont{footline}\insertshorttitle
  \end{beamercolorbox}%
  \begin{beamercolorbox}[wd=.33\paperwidth,ht=2.4ex,dp=1.6ex,right]{footline}%
    \usebeamerfont{footline}\textcolor{gblue300}{\insertframenumber}\,/\,\inserttotalframenumber\hspace*{1.5em}
  \end{beamercolorbox}%
  }%
  }%
  \vskip0pt%
